...
(defun dan/nvm-use-for ()
  ...
  (condition-case err
      (nvm-use-for)
    ('error
     (pcase (completing-read "Select a Node version:"
                             (append (nvm--installed-versions)
                                     (list (format "%s (%s)"
                                                   "system"
                                                   (s-trim (with-temp-buffer
                                                             (call-process-shell-command "/usr/bin/node -v" nil t)
                                                             (buffer-string)))) ))
                             nil t)
       ("system" (dan/nvm-deactivate))
       (node-version (nvm-use node-version))))))
#+end_src
