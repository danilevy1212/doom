...
Projectile can slow down remote connections when trying to find a remote name.

#+begin_src elisp
(defadvice! dan/disable-projectile-project-root-on-remote (org-fun &rest args)
  "Disable search of the projectile root name on remote hosts."
...
With the default settings, tramp can be a bit slow. Here's some hacks to fix it:

#+begin_src elisp
(after! tramp
  (setq remote-file-name-inhibit-cache 60 ;; Less frequent cache write on remote host
        tramp-completion-reread-directory-timeout 60 ;; Less frequent cache write on remote host
        tramp-verbose 1                              ;; Remove verbosity
        vc-ignore-dir-regexp (format "%s\\|%s\\|%s"
                                           vc-ignore-dir-regexp
                                           tramp-file-name-regexp
                                           "[/\\\\]node_modules"))) ;; Disable remote VC
#+end_src
...
The default ~completion-category-overrides~ messes with remote name completion. Until [[https://github.com/hlissner/doom-emacs/pull/5313][this PR]] is merged, this little hack will have to do.
...
(after! vertico
  ...
  (setq completion-category-overrides '((file (styles +vertico-basic-remote orderless partial-completion)))))
#+end_src
