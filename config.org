...
#+begin_src elisp
  :config
  (defadvice! dan/browse-url-chromium--parse-url (args)
    "Process a `lbry://' link so it can be opened with `browse-url-chromium'."
    :filter-args #'browse-url-chromium
    (let ((url (car args))
          (window (cdr-safe args)))
      (flatten-list
       (list (replace-regexp-in-string "^lbry:\/\/" "https://odysee.com/" url)
             window)))))
...
#+begin_quote
Spotify.el is a collection of extensions that allows you to control the Spotify application from within your favorite text editor.
#+end_quote
