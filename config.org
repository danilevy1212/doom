...
There is a bug in ~detached-init-projectile~ that prevents it from properly integrating. Here is my fix.

#+begin_src emacs-lisp :tangle yes
(after! detached
  (defadvice! dan/detached-init-projectile ()
    "Initialize integration with `projectile'."
    :override #'detached-init-projectile
    (after! projectile
...
Only activate it on ~term-mode~ buffers.
...
Hack for making ~flycheck-flow~ work with the ~lsp~ checker, taken from [[https://github.com/flycheck/flycheck/issues/1762][this issue]].
...
(after! flycheck
  (defun dan/flycheck-local-checker-get (fn checker property)
  ...
  (advice-add 'flycheck-checker-get
              :around 'dan/flycheck-local-checker-get)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (org-super-agenda-groups      '((:name "Started"
                                   :and (:todo "NEXT" :not (:tag ("consume") :scheduled future)))
                                  (:name "Today"
                                   :scheduled today :scheduled past :date today)
                                  (:name "Important"
