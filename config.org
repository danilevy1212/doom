...
An =emacs= alternative to the traditonal shell. From this shell, you have access to all of emacs internal functions and variables. The features of the =eshell= are too many to [[https://www.gnu.org/software/emacs/manual/html_mono/eshell.html][explain here]].
...
Alongside the reasonable defaults, this module offers tons of convinience commands, under the ~+default/~ prefix.
...
Unfortunatly, =doom= doesn't integrate the excellent =prescient= with =company-mode= in it's modules like it does for =ivy-mode=. First, we get the =company-prescient= package.
