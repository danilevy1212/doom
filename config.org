# -*- eval: (when (fboundp #'toc-org-mode) (not toc-org-mode)) (toc-org-mode)); -*-
...
=package.el=.  Package declarations blocks can be distinguished for only
...
In this file functions are stored that shouldn't be loaded until they're needed
and logic that should be autoloaded (evaluated very, very early at startup).
...
to their related configuration blocks. This can be identied by their
~;;;###autoload~ cookie. Autoloads blocks go into =autoload.el=. Here we just
declare it's lexical binding.
...
the packages from the different modules and in =package.el=. In other words, the
...
# TODO ~dan/nvm-deactivate~ interactive function, so I can go back to using system npm/node

# Get PATH, get NVM_DIR (default to "~/.nvm"), find path in PATH that contain NVM_DIR, delete it, setenv of PATH with result.
