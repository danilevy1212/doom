...
#+begin_src elisp :tangle autoload/awesomewm.el :mkdirp yes
;;;###autoload
...
To incorporate the same  behavior as ~projectile-switch-project~ when changing project:

#+begin_src elisp
(after! consult-projectile
  (setq +workspaces-switch-project-function #'consult-projectile--file)
  (plist-put! consult-projectile--source-projectile-project
...
#+begin_src elisp
(custom-set-faces!
  '(mode-line :family "Sarasa UI J" :height 0.9)
  '(mode-line-inactive :family "Sarasa UI J" :height 0.9))
...
Finally, ~dtache~ can integrate with ~alert.el~ to notify us when a session has becomed inactive.

#+begin_src elisp
(after! dtache
  (defun dan/dtache-inactive-session-notification (session)
  ...
  (setq dtache-notification-function #'dan/dtache-inactive-session-notification))
#+end_src
...
Do not activate in ~org-capture~ mode but leave it on after finalizing capture.

#+begin_src elisp
(after! org-capture
  (add-hook! org-capture-mode :append (lambda () (org-sticky-header-mode -1)))
  (add-hook! 'org-capture-prepare-finalize-hook (lambda () (org-sticky-header-mode +1))))
#+end_src
...
Make headers =foldable= and =quitable=.
...
Only the first important categories should open by default, fold the rest.

#+begin_src elisp
(after! (evil-org-agenda org-super-agenda)
  (add-hook! 'org-agenda-finalize-hook
             :append (when (and (org-agenda-check-type nil 'todo)
                                ;; Don't fold if we are filtering.
                                (not (or org-agenda-filter-form
                                         org-select-this-todo-keyword
                                         ;; Don't fold when narrowed
                                         (buffer-narrowed-p))))
                       (goto-char (point-min))
                       (re-search-forward "^$")
                       (forward-line)
                       (let ((inhibit-message t)
                             ...
                             (must-show-headers (cl-loop for group in (seq-take org-super-agenda-groups 4)
                                                         collect (plist-get group :name)))
                             (current-heading-function  (lambda ()
                                                          (let ((line-text  (buffer-substring-no-properties (point-at-bol)
                                                                                                            (point-at-eol))))
                                                            (string-trim line-text)))))
                         (while (/= last-point (point))
                           (setq last-point (point))
                           (unless (member (funcall current-heading-function) must-show-headers)
                             (origami-close-node buf (point)))
                           (origami-forward-fold-same-level buf (point)))))))
...
If we are filtering the agenda view, open up the nodes with results.

#+begin_src elisp
(after! (evil-org-agenda org-super-agenda)
  (defadvice! dan/org-super-agenda-open-nodes (&rest _)
    "Open all super agenda groups, under the right conditions."
    :after #'org-agenda-filter-apply
    (when (and (org-agenda-check-type nil 'todo)
               (or org-agenda-filter-form
                   org-select-this-todo-keyword)
               (not (buffer-narrowed-p)))
      (origami-open-all-nodes (current-buffer)))))
#+end_src

When quitting the agenda, remove all filters.

#+begin_src elisp
(after! (evil-org-agenda org-super-agenda)
  (defadvice! dan/org-agenda-quit-remove-filters (&rest _)
    "Remove filters when quitting the org agenda."
...
Currently, only used to fold ~org-super-agenda~ headers.
...
*** So Long Mode
=emacs= struggles with long lines. In ~emacs-version >= 28~, the ~so-long-statistics-excessive-p~ function has been reworked into a native super fast function to detect when ~so-long-mode~ is needed.
...
#+begin_src elisp
(add-hook! 'doom-first-first-buffer #'+global-word-wrap-mode)
#+end_src
...
#+begin_src elisp
(after! time
  (setq world-clock-list '(("America/Los_Angeles" "Santa Rosa")
