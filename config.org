...
With the ~detached-env~ script we get colored stdout for our detachedd commands, plus other goodies.

#+begin_src emacs-lisp :tangle yes
(after! detached
  (setq detached-env (expand-file-name "detached-env"
                                     (file-name-directory (locate-library "detached")))))
#+end_src
...
(use-package! run-command
  ...
  (defadvice! dan/run-command--run-detached-shell-command (command-line buffer-base-name)
    ...
    (let ((compilation-buffer-name-function
      ...
      (detached-shell-command command-line))))
#+end_src
