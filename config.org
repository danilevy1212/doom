...
Read RSS feeds in the comfort of Emacs. Thanks to [[https://github.com/skeeto/elfeed][elfeed]].
...
#+begin_src emacs-lisp :tangle autoload/awesomewm.el
;;;###autoload
...
Until [[https://github.com/doomemacs/doomemacs/issues/3817][completion stops working in eshell when typing quote characters]] is fixed, we apply the fix ourselves.  See [[https://github.com/doomemacs/doomemacs/issues/3817][here]].
...
Set the name of the =RSS= workspace.

#+begin_src emacs-lisp :tangle yes
(setq +rss-workspace-name "RSS")
#+end_src
...
Point =elfeed= to the org configuration file.
...
#+begin_src emacs-lisp :tangle autoload/eshell.el
...
I have to use =Intellij Idea= for some tasks work tasks. This function makes it possible to open the current file in =Intellij Idea=, provided the scripts from [[https://www.jetbrains.com/help/idea/working-with-the-ide-features-from-command-line.html#toolbox][Jetbrains Toolbox are installed and in path]].

#+begin_src emacs-lisp :tangle autoload/idea.el
...
Keybinding for the default agenda and file-local ~org-todo~ agenda.

#+begin_src emacs-lisp :tangle yes
(map! (:leader
       :prefix "o a"
       :desc "3 Day Org Agenda" "d" #'org-agenda-list
       :desc "Global Todo List" "t" #'dan/=org-todo-list
       :desc "Todo list here"   "T" #'dan/org-todo-list-current-file))
#+end_src

Key-bind ~org-agenda-follow-mode~.
...
Key-bind restrictions to the ~org-agenda~.

#+begin_src emacs-lisp :tangle yes
(map! (:after org
       :map org-mode-map :localleader
       :g "<" #'org-agenda-set-restriction-lock
       :g ">" #'org-agenda-remove-restriction-lock)
      (:after org-agenda :localleader
       :map org-agenda-mode-map
       :g "<" #'org-agenda-set-restriction-lock-from-agenda
       :g ">" #'org-agenda-remove-restriction-lock))
#+end_src
...
Show the TODOs of the current file in ~org-todo~ fashion.

#+begin_src emacs-lisp :tangle autoload/org.el
;;;###autoload
(defun dan/org-todo-list-current-file (&optional arg)
  "Same as `org-todo-list' but restricted to the current buffer."
  (interactive "P")
  (let* ((cbfn             (buffer-file-name (current-buffer)))
         (org-agenda-files (list cbfn)))
    (unless (and (eq major-mode 'org-mode)
                 cbfn)
      (user-error "Must be called within an .org file"))
    (org-agenda-set-restriction-lock 'file)
    (org-todo-list arg)
    (with-current-buffer org-agenda-buffer-name
      (add-hook! 'kill-buffer-hook :local t (org-agenda-remove-restriction-lock t)))))
#+end_src

Show the global TODO list in it's own workspace.

#+begin_src emacs-lisp :tangle autoload/org.el
(defvar +org-todo-list-workspace-name "Org TODOs")

;;;###autoload
(defun dan/=org-todo-list (&optional arg)
  "Like `org-todo-list', but the buffer is within it's own workspace."
  (interactive "P")
  (+workspace-switch +org-todo-list-workspace-name t)
  (unless (eq (buffer-local-value 'major-mode (window-buffer (selected-window)))
              'org-agenda-mode)
    (doom/switch-to-scratch-buffer)
    (org-todo-list arg)
    (with-current-buffer org-agenda-buffer-name
      (add-hook! 'kill-buffer-hook :local t
                 (+workspace-delete +org-todo-list-workspace-name)))
    (+workspace/display)))
  #+end_src
...
Add support for links to =man= pages and =info= internal documentation.

#+begin_src emacs-lisp :tangle yes
(after! ol
  (dolist (p '(ol-man ol-info))
    (require p)))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
  :custom
  (org-agenda-sorting-strategy '((agenda habit-down scheduled-up deadline-up ts-up time-up priority-down)
                                 (todo habit-down scheduled-up priority-down)
                                 (tags habit-down scheduled-up priority-down)
                                 (search habit-down scheduled-up priority-down))))
#+end_src
...
The ~dan/org-super-agenda--date-today~ selector picks agenda items with timestamps in the past or today. Similarly, ~dan/org-super-agenda--date-soon~ select items that are soon.

#+begin_src emacs-lisp :tangle yes
(after! org-super-agenda
  (defun dan/org-super-agenda--date-today (item)
    "Select agenda ITEM whose date is today or with a daterange that includes today."
  ...
  (defun dan/org-super-agenda--date-soon (item)
    "Select agenda ITEM whose date is within 3 days from today."
    (let ((this-marker (org-find-text-property-in-string 'org-marker item)))
      (when-let ((date          (org-entry-get this-marker "TIMESTAMP"))
                 (soon-end-date (+ 3 (org-today))))
        (if (string-match-p "--" date)
            (cl-destructuring-bind (date-start date-end) (split-string date "--" t)
              (and (>= soon-end-date
                      (org-time-string-to-absolute date-start))
                  (<= soon-end-date
                      (org-time-string-to-absolute date-end)))))
        (>= soon-end-date (org-time-string-to-absolute date))))))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
(after! org-super-agenda
  (defun  dan/org-super-agenda--soon-limit ()
...
(after! org-super-agenda
  (setq org-super-agenda-groups      `((:name "Started"
                                       ...
                                       (:name "Today"
                                        ...
                                        :scheduled today
                                        :scheduled past
                                        :pred dan/org-super-agenda--date-today)
                                       (:name "Soon"
                                        :scheduled (before ,(dan/org-super-agenda--soon-limit))
                                        :pred dan/org-super-agenda--date-soon
                                        :transformer dan/org-super-agenda--transformer-show-scheduled-or-deadline)
...
#+begin_src emacs-lisp :tangle autoload/telega.el
...
#+begin_src snippet :tangle snippets/org-mode/文法
# -*- mode: snippet -*-
# name: 文法の事項
# key: bunpou
# --
...
#+begin_src snippet :tangle snippets/org-mode/new_proj
# -*- mode: snippet -*-
# name: New project
# key: project
# --
...
#+begin_src snippet :tangle snippets/org-mode/new_package
# -*- mode: snippet -*-
# name: New package
# key: package
# --
...
#+begin_src snippet :tangle snippets/org-mode/new_autoload
# -*- mode: snippet -*-
