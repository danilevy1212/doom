# -*- eval: (when (and (fboundp #'toc-org-mode) (not toc-org-mode)) (toc-org-mode)); -*-
...
# TODO Installation of the config may require to manually uncomment 'literate' module and run doom sync, create script for that?
# TODO Explore using defcli! to install with the doom cli utilily.
# TODO defcli! documentation is a bit out ouf date, another contributing opportunity?
...
# TODO [[https://github.com/p3r7/awesome-elisp][Check awesome-lisp or keep it as a reference.]]
...
folder there are several file which define functions that shouldn't be loaded
...
Let's choose our *monospaced* font, /Hack/ goodness:
...
# TODO Perspective Heading, customize
...
# TODO https://github.com/m-cat/org-recur#time-of-day Utilizar esto para los schedule clases de japones.
...
# #+begin_src elisp :tangle package.el
# (package! org-recur)
# #+end_src
...
# TODO Look into Auto-Fill-Mode for org buffers.
...
# TODO Prerequesites Heading

# Requires =emacs_mozc_helper=, this is how you install it:

# TODO Document [[https://github.com/rzl24ozi/mozc-emacs-helper-module][インストール]] instructions, maybe create elisp script for it?
# TODO If realiable, this could be added to the =japanese= module, as a =+mozc= flag.
...
[[file:~/.config/emacs/docs/getting_started.org::*=doctor.el=][=doctor.el=]] EXPLAIN THIS AND USE TO DETECT NVM
...
If nvm wasn't activated previously, PATH will remain unchanged."
  ...
  (when nvm-current-version
    (let* ((node-version-dir (expand-file-name "bin/" (-last-item nvm-current-version)))
           (old-path-list (parse-colon-path (getenv "PATH")))
           (new-path-list (--remove-first (s-equals-p node-version-dir it) old-path-list))) ;; TODO Use --reject with path-regex like in original package.
...
# TODO ~dan/nvm-use~ interactive wrapper. If a .nvmrc can't be found in ~default-directory~, then prompt user.

# Candidates: (nvm--installed-version) + system node (would be nice to show the version) (If it exists)

# If user selects 'system', call ~dan/nvm-deactivate~
# If user selects a version, call ~(nvm-use *version*)~
...
#     Search in PATH, filtered of NVM_DIR, for node, and run

#     #+begin_src elisp :tangle no
# (with-temp-buffer
#   (list :exit-status (call-process-shell-command (concat system-node " -v") nil t)
#         :output (buffer-string)))
#     #+end_src

#     Then just check with ~plist-get~ for exit status 0 and read output (~s-trim~-ed).

#     ~system-node~ can be obtained with ~expand-file-name~
