...
- TODO :: Task that needs to be done. Needs a SCHEDULED or DEADLINE date, else it's just a pipe dream.
- NEXT :: Task that has already started.
- PROJ :: Task that holds a collection of related tasks, events and ideas.
...
- IDEA :: A note that could turn into a task or an entire project, needs refinement.
- DONE :: Completed task.
